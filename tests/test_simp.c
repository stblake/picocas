...
void test_trigtoexp() {
    ...
    printf("testing %s\n", "TrigToExp[Sinh[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Sinh[x]]", "1/2 E^x - 1/2 E^(-1 x)", 0);
    printf("testing %s\n", "TrigToExp[Cos[x]+I Sin[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Cos[x]+I Sin[x]]", "E^((I) x)", 0);
    printf("testing %s\n", "TrigToExp[Tanh[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Tanh[x]]", "-E^(-1 x)/(E^x + E^(-1 x)) + E^x/(E^x + E^(-1 x))", 0);
    ...
    printf("testing %s\n", "TrigToExp[Sinh[x]<=11&&ArcTan[x]==7]"); fflush(stdout); assert_eval_eq("TrigToExp[Sinh[x]<=11&&ArcTan[x]==7]", "1/2 E^x - 1/2 E^(-1 x) <= 11 && (-1/2*I) Log[1 + (I) x] + (1/2*I) Log[1 + (-I) x] == 7", 0);
    ...
    // Hyperbolics
    printf("testing %s\n", "TrigToExp[Cosh[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Cosh[x]]", "1/2 E^x + 1/2 E^(-1 x)", 0);
    printf("testing %s\n", "TrigToExp[Coth[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Coth[x]]", "E^x/(-E^x + E^(-1 x)) + E^(-1 x)/(-E^x + E^(-1 x))", 0);
    printf("testing %s\n", "TrigToExp[Sech[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Sech[x]]", "2/(E^x + E^(-1 x))", 0);
    printf("testing %s\n", "TrigToExp[Csch[x]]"); fflush(stdout); assert_eval_eq("TrigToExp[Csch[x]]", "2/(-E^(-1 x) + E^x)", 0);
...
void test_exptotrig() {
    ...
    printf("testing %s\n", "ExpToTrig[TrigToExp[Coth[x]]]"); fflush(stdout); assert_eval_eq("ExpToTrig[TrigToExp[Coth[x]]]", "-1 Cosh[x] Csch[x]", 0);
